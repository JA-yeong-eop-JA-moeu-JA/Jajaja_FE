...
export default function BaseCheckbox({ checked, onClick, message, textClassName, disabled = false }: IBaseCheckboxProps) {
  return (
    <label className={`flex items-center gap-4 cursor-pointer text-body-regular ${disabled ? 'opacity-50 cursor-not-allowed' : ''}`}>
      ...
      {message && <span className={textClassName ?? 'text-body-regular'}>{message}</span>}
    </label>
  );
}
