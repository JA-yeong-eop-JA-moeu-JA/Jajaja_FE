  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <main className="flex-1 flex flex-col gap-8 overflow-y-auto">
        ...
        <section className="bg-white pt-4 px-4 pb-8 border-b border-b-4 border-b-black-1">
          ...
          <div className="flex gap-0">
            <div className="flex flex-col gap-2 text-sm text-gray-400 font-body-regular w-18">
              <span>송장 번호</span>
              <span>택배사</span>
            </div>
            <div className="flex flex-col gap-2 text-sm text-black font-body-regular">
              <div className="flex items-center gap-2">
                <span>{dummyDeliveryData.invoiceNumber}</span>
                <button
                  onClick={() => navigator.clipboard.writeText(dummyDeliveryData.invoiceNumber)}
                  className="text-sm leading-5 font-normal underline underline-offset-0 text-[color:var(--color-green)] hover:text-[#87C500]"
                >
                  복사하기
                </button>
              </div>
        ...
        {/* 안내 문구 */}
        <section className="flex flex-col items-center text-center text-sm font-body-regular px-4">
      ...
      <div className="px-2 pb-0 pt-2 bg-white">
        <Button kind="basic" variant="outline-orange" className="w-full" onClick={() => (window.location.href = '/')}>
          배송 현황 확인
        </Button>
      </div>
    </div>
