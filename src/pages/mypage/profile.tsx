...
export default function Profile() {
  const { nickname, phoneNumber, email, profileImage } = profileData;
    ...
    <div className="w-full h-screen flex flex-col items-center justify-between">
      <div className="w-full">
        ...
        <div className="w-full flex flex-col items-center justify-center px-4 pt-2 pb-11">
          {profileImage ? (
            <img src={profileImage} alt="프로필 이미지" className="w-30 h-30 rounded-full object-cover" />
          ) : (
            <DefaultProfile className="w-30 h-30" />
          )}
          <button className="py-3.5" onClick={() => {}}>
